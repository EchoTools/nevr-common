...
func main() {
	...
	// Test v2 envelope with 2 players
	v2Env2 := createV2Envelope(2)
	v2Data2, _ := proto.Marshal(v2Env2)
	fmt.Printf("V2 Envelope (Frame) with 2 players: %d bytes\n", len(v2Data2))

	// Test v2 envelope with 10 players
	v2Env10 := createV2Envelope(10)
	v2Data10, _ := proto.Marshal(v2Env10)
	fmt.Printf("V2 Envelope (Frame) with 10 players: %d bytes\n", len(v2Data10))

	// Test v2 header envelope
	v2Header := createV2HeaderEnvelope()
	v2HeaderData, _ := proto.Marshal(v2Header)
	fmt.Printf("V2 Envelope (Header): %d bytes\n", len(v2HeaderData))
...
func createV2Envelope(playerCount int) *telemetryv2.Envelope {
	discHolder := int32(0)
	echoFrame := &telemetryv2.EchoArenaFrame{
	...
	for i := 0; i < playerCount; i++ {
		echoFrame.Players[i] = &telemetryv2.PlayerState{
			Slot: int32(i),
			Head: &spatial.Pose{

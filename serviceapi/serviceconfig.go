...
func NewNakamaServiceConfig(host string, port int, wss bool, https bool, XPID uuid.UUID, Username string, DiscordID string, Password string, Guilds []string, Channels []uuid.UUID, HttpKey string, ServerKey string) *NakamaServiceConfig {
	scheme := "http"
	if https {
		scheme = "https"
	}
	...
	if XPID != uuid.Nil {
		wsURL.RawQuery += "&user_id=" + XPID.String()
	}

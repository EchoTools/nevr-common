...
func TestMarshal(t *testing.T) {
	...
	messages := make([]EasyStreamer, len(packets))
...
func TestUnmarshalUnknownSymbolPacket(t *testing.T) {
	...
	errSym := &ErrSymbolNotFound{}
	data := append(testMessage, testMessage...)
	data[10] = 0x00
	packet, err := ParsePacket(data)
	if !errors.As(err, errSym) {
		t.Errorf("got %v, want %d", err, errSym.SymbolHash)
		return
	}
	if len(packet) != 0 {
...
func TestWrapBytes(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			"Valid",
			args{
				SymbolOf(&TCPConnectionUnrequireEvent{}),
				[]byte{
					0xfe,
				},
			},

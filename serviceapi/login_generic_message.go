...
func NewGenericMessage(session uuid.UUID, acctId uint64, messageType Symbol, otherXPID XPID, partyData GenericMessageData) *GenericMessage {
...
func (m *GenericMessage) SessionID() uuid.UUID {
	return m.Session
}

func (m GenericMessage) String() string {
	return fmt.Sprintf("GenericMessage{Session: %s, AcctId: %d, OVRSymbol: %d, OtherXPID: %s, RoomId: %d, PartyData: %v}", m.Session, m.AcctId, m.MessageType, m.OtherXPID.Token(), m.RoomID, m.PartyData)
}

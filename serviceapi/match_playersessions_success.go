...
func NewLobbyEntrant(xpid XPID, matchingSession uuid.UUID, playerSession uuid.UUID, playerSessions []uuid.UUID, teamIndex int16) *LobbyEntrant {
	return &LobbyEntrant{
...
func (m LobbyEntrantsV2) String() string {
	return fmt.Sprintf("%T(unk0=%d, evr_id=%v, entrant_id=%s)", m, m.Unk0, m.XPID, m.EntrantID)
}

...
type Streamer interface {
	Stream(s Streamer) error
}

type EasyStreamer interface {
...
func (e ErrSymbolNotFound) Error() string {
	return fmt.Sprintf("symbol not found: %x", e.SymbolHash)
}

func (e ErrSymbolNotFound) Symbol() Symbol {
	return Symbol(e.SymbolHash)
}

func (e ErrSymbolNotFound) Is(err error) bool {
	if e2, ok := err.(ErrSymbolNotFound); ok {
		return e.SymbolHash == e2.SymbolHash
	}
	return false
}
...
var (
	...
	SymbolTypes = map[uint64]EasyStreamer{
		...
		*/
		...
		0x43e6963ac76beee4: (*TCPConnectionUnrequireEvent)(nil),
		0x0fe57a8b3980e660: (*TCPConnectionRequireEvent)(nil),
...
func Marshal(msgs ...EasyStreamer) ([]byte, error) {
	var errs error
	b := make([]byte, 0)
	for _, m := range msgs {
		// Encode the message.
		...
		if !ok {
			errs = errors.Join(ErrSymbolNotFound{SymbolHash: sym}, fmt.Errorf("message type %T", m), errs)
			continue
		}
...
func ParsePacket(data []byte) ([]EasyStreamer, error) {
	var err error

	// Split the packet into individual messages.
	chunks := bytes.Split(data, MessageMarker)

	messages := make([]EasyStreamer, 0, len(chunks))
	...
	for _, b := range chunks {
		...
		message := reflect.New(reflect.TypeOf(typ).Elem()).Interface().(EasyStreamer)
...
func SymbolOf(m EasyStreamer) Symbol {
	typ := reflect.TypeOf(m).String()
	sym, ok := reverseSymbolTypes[typ]
	if !ok {
...
func MessageTypeOf(s Symbol) EasyStreamer {
	if m, ok := SymbolTypes[uint64(s)]; ok {
		// return a new instance of the message type
		return reflect.New(reflect.TypeOf(m).Elem()).Interface().(EasyStreamer)
	}
	return nil
}
